...
/*Are suicidal thoughts more prevalent in international students than the domestic students*/
SELECT inter_dom, COUNT(suicide)
FROM students
WHERE suicide = 'yes'
GROUP BY inter_dom;

/*Any correlation of mental health scores and region the students are from*/
SELECT region, COUNT(*) AS num_of_students, AVG(toSC) AS avg_tosc, AVG(toAS) AS avg_toAS, AVG(toDep) AS avg_toDep
FROM students
WHERE region IS NOT NUll
GROUP BY region
ORDER BY num_of_students DESC	

/*Any correlation between age of students and their mental health scores*/
SELECT age, AVG(toSC) AS avg_tosc, AVG(toAS) AS avg_toAS, AVG(toDep) AS avg_toDep
FROM students
WHERE inter_dom = 'Inter'
GROUP BY age;